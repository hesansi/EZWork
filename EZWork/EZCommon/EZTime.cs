...
namespace EZWork
{
    public class EZTime : EZSingletonStatic<EZTime>
    {
        private EZTime() { }
...
//            Debug.Log("~~~ CreateTimer() target: "+handler.method.Target+" UUID "+handler.UUID);
        ...
        private void clear(Delegate method, bool pool = true)
        {
            TimerHandler handler = _handlers.FirstOrDefault(t => t.method == method);
            if (handler != null) {
//                Debug.Log("~~~ clear() target: "+handler.method.Target+" UUID "+handler.UUID);
